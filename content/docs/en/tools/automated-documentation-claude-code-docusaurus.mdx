...
category: "tools"
language: "en"
tags: ["claude-code", "docusaurus", "documentation", "automation", "ci-cd", "github-actions", "agents"]
---
...
Here's the complete pipeline configuration that you can customize for your needs:
...
You can create custom agent instructions by modifying the `.claude/agents/docusaurus-expert.md` file:

```markdown
# Docusaurus Expert Agent

You are an expert in Docusaurus documentation and technical writing.

## Your Role
- Analyze code changes and generate appropriate documentation
...
The system can be extended to support multiple documentation formats:

```yaml
# Support for different doc formats
- name: Update documentation
  run: |
    case "$DOC_FORMAT" in
      "docusaurus")
...
1. **Infinite Loop Prevention**: Always exclude your documentation directory
2. **API Rate Limits**: Consider implementing delays between API calls
3. **Large Repositories**: Use selective file monitoring to reduce processing time
...
1. **Set up the basic Docusaurus site** following the installation guide
2. **Install the Docusaurus Expert agent** using the provided command
3. **Configure the GitHub Actions workflow** with your project-specific settings
4. **Test the workflow** with a small change to verify it works correctly
5. **Customize the agent prompts** to match your documentation style
6. **Monitor and refine** the system based on the quality of generated documentation

With this setup, your documentation will automatically stay synchronized with your codebase, ensuring your team and users always have access to current, accurate information.